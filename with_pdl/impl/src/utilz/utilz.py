...
class Util():
    ...
    def setDataLoader(self, flist, args, phase='train'):
        partition = self.getPartition(args, flist)
        seqdataset = SeqDataset(flist
                                , args.max_len
                                , args.window_size
                                , args.fp_slice_size
                                , args.do_truncate
                                , batch_padding=args.batch_padding
                                , binaries_location=args.binaries_location
                                , batch_size=args.batch_size
                                , corpus_ram=partition)
        
        dl = DataLoader(seqdataset
                        , batch_size=args.subloader_batch_size
                        , shuffle=True
                        , num_workers=args.num_workers
                        , persistent_workers=False #True
                        , collate_fn=seqdataset.collater
                        , pin_memory=False)
        ...
        for id in range(len(flist)):
            ...
            try:
               with open(args.binaries_location + _file, 'rb') as f:
                ...
                print("Error in generating dynamic partition: ", str(e))
        return corpus
