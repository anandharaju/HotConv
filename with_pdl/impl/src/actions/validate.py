...
def validate(batch_times, epoch_stats, args, ghandle, utilObj, mode, model):
        ...
        while num_samples_processed < population:
            flist = all_files[num_samples_processed:num_samples_processed + args.partition_size]
            args = utilObj.setDataLoader(flist, args, phase=mode)
            print("-----------> Partition : ", int(num_samples_processed / args.partition_size) + 1, "samples:", len(flist))
            with torch.no_grad():
                for batch_data in tqdm.tqdm(args.loader):
